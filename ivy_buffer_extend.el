...
(setq ivy--buffer-info-fns
  (list
   ...
   (ivy--buffer-make-info-fn
    ...
    (shorten-directory (abbreviate-file-name
			(or (with-current-buffer
				buffer_object (if (buffer-file-name) default-directory)) ""))
   ...
   (ivy--buffer-make-info-fn
    'process
    (if (get-buffer-process buffer_object) ":" "")
    '"")
...
(defvar ivy-buffer-format-faces
  (list
   ...
   '(dir . (font-lock-function-name-face . nil))
...
(defface ivy-dir-face '((t :inherit 'feebleline-dir-face :bold nil :family "Noto Sans"))
  "Feebleline filename face."
  :group 'feebleline)

(custom-set-faces '(ivy-dir-face ((t :inherit 'feebleline-dir-face
                                     :bold nil
                                     :italic nil
                                     :height 0.75
                                     :family "Inconsolata"))))
...
(setq ivy-buffer-format-faces
  (list
   ...
   '(dir . (ivy-dir-face . ivy-dir-face))
   '(process . (font-lock-variable-name-face . nil))
   '(mode . (feebleline-dir-face . nil))
   ))
