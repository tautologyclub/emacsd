...
# Change prompt, but only for 'real' terminal. This is to be nice to emacs
# terminal emulator...
if [ ! "$0" = "/bin/bash" ]; then
    PS1=\\w\$\  # Comment to remove training whitespace autoremoval

    # Change terminal title to $pwd but only two deepest dirs
...
##### # NOTE: Moved this to /etc/profile.d/caps_super_switch.sh
...
# TODO: put $pwd into clipboard
#       move newest file in ~/Downloads to $pwd

# python
# export PYTHONPATH="${PYTHONPATH}:~/workspace/dota2bet"

# gcalcli agenda "`date`" "$(date -d "+1 days")"
...
if [ -z "$(pgrep xcape)" ]; then
    xcape -e 'Shift_L=F10;Super_L=F11;Control_L=F12;Alt_R=F9;Shift_R=Control_L|x;Mode_switch=F8'
fi
