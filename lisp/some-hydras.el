...
(defhydra hydra-lazy (:columns 4 :color blue)
  " -- lazy hydra -----------"
  ("nr" (lambda () (interactive) (shell-command "nrfjprog --reset"))
   "\"nrfjprog --reset\"")
  ("jg" (lambda () (interactive) (shell-command "systemctl --user start jlink_gdb"))
   "JLinkGDB @ nRF52")
  ("JG" (lambda () (interactive) (shell-command "systemctl --user stop jlink_gdb"))
   "JLinkGDB stop")
  ("jl" (lambda () (interactive) (shell-command "jllog -device NRF52 -if SWD -speed 4000 -rttchannel 0 /tmp/jlink.cap"))
   "JLRTTLogger @ nRF52")
  ("ps" (lambda () (interactive) (profiler-start 'cpu)) "profiler-start")
...
-- PROJECTILE ------------------------------------------------------------------"
  ...
  ("C-t" projectile-test-project "test")
  ...
  ("r"   counsel-projectile-rg "rg") ;; tmp, counsel-rg kinda broken...?
...
(defun benjamin/nav-mode-toggle-hl-line ()
  "Turn on 'hl-line-mode' if 'nav-mode' is active."
  (if nav-mode
      (progn (hl-line-mode))
    (hl-line-mode -1)))
...
replacements. "
  ...
  (if nav-mode
      (progn (let ((inhibit-message t))
             ...
             (set-face-attribute (make-local-variable 'hl-line) nil :background "pink")

             ;; todo - buffer-local faces...
             ;; (setq nav-mode--prev-region-bg (face-attribute 'region :background))
             ;; (set (make-local-variable 'region) nav-mode-region-face)
             (setq-local cursor-type '(hbar . 7)))
    ;; (hl-line-mode -1)
    (setq-local cursor-type nav-mode--prev-cursor-type)
    ;; (setq region nav-mode--prev-region-face)
...
(define-key nav-mode-map (kbd "R") "") ;;
(define-key nav-mode-map (kbd "T") "") ;;
(define-key nav-mode-map (kbd "Y") "") ;;
...
(define-key nav-mode-map (kbd "H") (lambi (let ((deactivate-mark))) (mark-paragraph)))
...
(define-key nav-mode-map (kbd "?") 'describe-key)
